...
document.addEventListener('DOMContentLoaded', () => {
	...
	photoList.addEventListener('scroll', event => {
		...
		if (photoList.scrollWidth === photoList.clientWidth + photoList.scrollLeft) {
			photoScrollRight.classList.remove('active');
			canScrollRight = false;
		}

		if (!canScrollRight && photoList.scrollWidth > photoList.clientWidth + photoList.scrollLeft) {
	...
	window.addEventListener('scroll', event => {
		...
		navFade.seek(navFade.duration * (Math.max(0, scrollTop - headerHeight + 100) / 50));
	});
